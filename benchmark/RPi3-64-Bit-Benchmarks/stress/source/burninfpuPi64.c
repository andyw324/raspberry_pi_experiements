...
gcc-6 burninfpuPi64.c cpuidc.c -lm -lrt -O3 -march=armv8-a  -o burninfpuPi64 
...
*/
 ...
 size_t  size_x;
 int     words     = 3200;      // E Number of words in arrays
...
void triadplus2(int n, float a, float b, float c, float d, float e, float f, float g, float h, float j, float k, float l, float m, float o, float p, float q, float r, float s, float t, float u, float v, float w, float y, float *x)
 ...
 void triadplus(int n, float a, float b, float c, float d, float e, float f, float *x)
 {
    ...
    for (jj=0; jj<repeats; jj++)
    {
     for(i=0; i<n; i=i+16)
     {
       ...
       x[i+1] = (x[i+1]+a)*b-(x[i+1]+c)*d+(x[i+1]+e)*f;
       x[i+2] = (x[i+2]+a)*b-(x[i+2]+c)*d+(x[i+2]+e)*f;
       x[i+3] = (x[i+3]+a)*b-(x[i+3]+c)*d+(x[i+3]+e)*f;
       x[i+4] = (x[i+4]+a)*b-(x[i+4]+c)*d+(x[i+4]+e)*f;
       x[i+5] = (x[i+5]+a)*b-(x[i+5]+c)*d+(x[i+5]+e)*f;
       x[i+6] = (x[i+6]+a)*b-(x[i+6]+c)*d+(x[i+6]+e)*f;
       x[i+7] = (x[i+7]+a)*b-(x[i+7]+c)*d+(x[i+7]+e)*f;
       x[i+8] = (x[i+8]+a)*b-(x[i+8]+c)*d+(x[i+8]+e)*f;
       x[i+9] = (x[i+9]+a)*b-(x[i+9]+c)*d+(x[i+9]+e)*f;
       x[i+10] = (x[i+10]+a)*b-(x[i+10]+c)*d+(x[i+10]+e)*f;
       x[i+11] = (x[i+11]+a)*b-(x[i+11]+c)*d+(x[i+11]+e)*f;
       x[i+12] = (x[i+12]+a)*b-(x[i+12]+c)*d+(x[i+12]+e)*f;
       x[i+13] = (x[i+13]+a)*b-(x[i+13]+c)*d+(x[i+13]+e)*f;
       x[i+14] = (x[i+14]+a)*b-(x[i+14]+c)*d+(x[i+14]+e)*f;
       x[i+15] = (x[i+15]+a)*b-(x[i+15]+c)*d+(x[i+15]+e)*f;
    }
   }
 }
 ...
 {         
          ...
          switch (toupper(argv[i][0]))
          {
               case 'K':
                ...
                if (argc > i+1)
                {
                   ...
                   i=i+1;
                   if (param1 > 500000) param1 = 500000;
    ...
    printf("    Burn-In-FPU %s %s\n", version, timeday);
    fprintf(outfile, "    Burn-In-FPU %s %s\n", version, timeday);
    ...
    for (p=0; p<passes; p++)
    {
        ...
        if (x_cpu  == NULL)
        {
            printf(" Can't Allocate %d Memory Words - ERROR WILL EXIT\n", words);
            printf(" Press Enter\n");
            g  = getchar();
            exit(1);
        }
        ...
        // Print results
        fprintf(outfile, "%7d %10d %5d %8d %10.2f %8.0f ", p+1, words, opwd, repeats * repeat2, secs, mflops);
        printf("%3d%5d%10d %5d %8d %10.2f %8.0f ", testlog, p+1,words, opwd, repeats * repeat2, secs, mflops);
        isok1  = 0;
        for (i=0; i<words; i++)
        {
          if (one != x_cpu[i])
